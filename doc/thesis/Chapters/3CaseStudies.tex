...
Still, 
% and trained the models with the "minimize" function, by using a "likLogistic" likelihood function and the Expectation Propagation (EP) as inference method.
...
% although is not able to separate correctly a data point.  
%%% CONTROLLARE QUA SE METTERE L'ULTIMA FRASE 
...
The final values can be found in Sec.~\ref{sec:classifier_hyperparameters} of the Appendix.\\
...
\begin{figure}[h]
  ...
  \caption{Time-to-threshold emulator: training (red), validation (blue) and test (green) datasets and the emulator (mesh) predicting the time-to-threshold ($t_!$) in regions where there are no observations. The emulator interpolates all of the training points (red and blue ones). The extrapolation far outside the observations boundaries are meaningless. These are already discarded by the classifier.}
  \label{fig:emulator}
\end{figure}
...
The set of mean, covariance and likelihood functions with which the emulator was obtained is:\\
...
The performance of the emulator was assessed by computing MAE and RMSE on the test dataset.  These provides an estimation of the out-of-sample error, and is reported in Tab.~\ref{tab:emulator_performance}.
...
In early flood warning system, a crucial point is to not underestimate the danger of floods.
In the case a flood alert is issued but no flood occur, minor economic loss could be related to deployment of temporary flood control measures or the cost of population evacuations, but nobody would be armed. 
...
Our emulation-based early flood warning system is composed by a \emph{classifier} which says if an alert should be emitted, and by an \emph{emulator} which estimates the time period before a specific discharge is exceeded in case an alert is emitted by the classifier.

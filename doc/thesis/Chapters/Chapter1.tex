...
\begin{itemize}
  \item Intro: 2 -- 3 pages 
\end{itemize}

\section{Background}
\begin{itemize}
  \item the problem: inundations and climate change
  \item the solution with its deficiencies: simulation
  \begin{itemize}
    \item calibration of models
  \end{itemize}
  \item from emulation to simulation
  \item simulation and problems related
  \item Canton Lucern and overland flow maps
\end{itemize}
...
Design and implementation of these need preliminary study of the situation based on available data and information.
...
These are mainly based on topographical data of the catchment in question, which are then refined by assigning friction coefficients values, infiltration capacity values, hydraulic conductivity values, porosity values to the different regions of the catchment.
Such models need to be calibrated.
This means that by using optimization algorithms the parameters of the model are varied within a certain range in order to reproduce at best a recorded output (e.g. river outlet hydrograph) generated from "known" initial conditions (e.g. initial soil saturation) and inputs (e.g. the recorded hyetograph).
...
\autocite{denzler_quantensprung_2017}

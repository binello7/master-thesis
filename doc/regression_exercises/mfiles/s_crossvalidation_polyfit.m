...
# 3.2.b How to calculate dy? dp is clear, it is the standard deviation of the p coefficients.
#       Why compute it as sqrt of the sum of squared values? What about abs() e.g.?
...
p_est_jk_f = @(x, y, loo, ln) arrayfun (@(i) polyfit (x(loo(:,i)), y(loo(:,i)), ln), 1:length(x), 'uniformoutput', false);
...
function sdjk = jk_stdv (p)
  n = length (p);
  p_m = mean (p);
  sdjk = sqrt ((n-1) / n * sum (cell2mat (transpose (arrayfun (@(i) ((p(i,:) - p_m).^2), 1:n, 'uniformoutput', false))), 1));
endfunction
...
#dp_pfit      = sqrt (diag (s.C)/s.df)*s.normr;

## method: jackknife
#p_est_jk = p_est_jk_f (X, Y, ind, elm);
#p_est_jk = p_est_jk.';
#p_est_jk = cell2mat (p_est_jk);
#dp_jk = transpose (jk_stdv (p_est_jk));
